 ...
 * File Name    : define.h
 * Version      : 7.30
 * Description  : RZ/V2L DRP-AI Sample Application for Darknet-PyTorch YOLOv5 MIPI Camera version
 ***********************************************************************************************************************/
 ...
 *
 * HOW TO COMPILE MODEL WITH DRP-AI TRANSLATOR (inside Docker):
 *   /opt/drp-ai_translator_release/DRP-AI_Translator \
 *       -i yolov5s.onnx -o yolov5_onnx --config config.json
 *
 * The output prefix "yolov5_onnx" must match drpai_prefix below.
 * On the board, copy the output folder to /home/root/ so the app can
 * load:  yolov5_onnx_aimac_desc.bin, yolov5_onnx_weight.dat, etc.
 *
 * TO USE A DIFFERENT NAME: change drpai_prefix to match your translator
 * output prefix, e.g. "myssd_cam" if you ran -o myssd_cam.
 */
const static std::string ai_name = " YoloV5s";
const static std::string drpai_prefix = "yolov5_onnx"; /* <-- CHANGE THIS to match DRP-AI Translator -o argument */
...
const static uint32_t num_inf_out = (NUM_CLASS + 5) * NUM_BB * num_grids[0] * num_grids[0] + (NUM_CLASS + 5) * NUM_BB * num_grids[1] * num_grids[1] + (NUM_CLASS + 5) * NUM_BB * num_grids[2] * num_grids[2];
...
const static std::string drpai_prefix = "yolov3_onnx";
/* Class labels to be classified */
const static std::string label_list = "coco-labels-2014_2017.txt";
/* Empty since labels will be loaded from label_list file */
static std::vector<std::string> label_file_map = {};
...
const static std::string drpai_prefix = "yolov2_onnx";
...
const static std::string drpai_prefix = "tinyyolov3_onnx";
/* Class labels to be classified */
const static std::string label_list = "coco-labels-2014_2017.txt";
/* Empty since labels will be loaded from label_list file */
static std::vector<std::string> label_file_map = {};
...
/* Number of DRP-AI output */
const static uint32_t num_inf_out = (NUM_CLASS + 5) * NUM_BB * num_grids[0] * num_grids[0] + (NUM_CLASS + 5) * NUM_BB * num_grids[1] * num_grids[1];
/* Anchor box information */
const static double anchors[] =
    {
        10, 14,
...
const static std::string drpai_prefix = "tinyyolov2_onnx";
/* Pascal VOC dataset label list */
const static std::vector<std::string> label_file_map = {"aeroplane", "bicycle", "bird", "boat", "bottle", "bus", "car", "cat", "chair", "cow", "diningtable", "dog", "horse", "motorbike", "person", "pottedplant", "sheep", "sofa", "train", "tvmonitor"};
