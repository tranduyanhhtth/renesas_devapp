...
***********************************************************************************************************************/
/***********************************************************************************************************************
* File Name    : sample_app_yolo_cam.cpp
* Version      : 7.30
* Description  : RZ/V2L DRP-AI Sample Application for Darknet-PyTorch YOLO Camera version
***********************************************************************************************************************/
...
static std::atomic<uint8_t> inference_finish (0);
...
void R_Post_Proc(float* floatarr)
{
	float new_w, new_h;
	        ...
	        for (b = 0;b<NUM_BB;b++)
	        {
	            for (y = 0;y<num_grid;y++)
	            {
	                for (x = 0;x<num_grid;x++)
	                {
	...
	#elif defined(YOLOV3) || defined(TINYYOLOV3)
	                    ...
	                    for (i = 0;i < NUM_CLASS;i++)
	                    {
#if defined(YOLOV3) || defined(TINYYOLOV3) || defined(YOLOV5)
	    ...
	    filter_boxes_nms(det, (int)det.size(), TH_NMS);
...
void draw_bounding_box(void)
{
    ...
    for (i = 0; i < det.size(); i++)
    {
        ...
        img.draw_rect((int32_t)det[i].bbox.x, (int32_t)det[i].bbox.y, (int32_t)det[i].bbox.w, (int32_t)det[i].bbox.h, result_str.c_str());
    }
    return;
}
