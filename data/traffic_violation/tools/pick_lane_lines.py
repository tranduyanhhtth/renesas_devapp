...
class OpenCVYamlDumper(yaml.SafeDumper):
...
DEPLOY_CONFIG_PATH = os.path.normpath(os.path.join(SCRIPT_DIR, "..", "exe_v2l", "config.yaml"))
...
class TrafficAnnotator:
    ...
    def save_config(self):
        ...
        lane_shapes = [
            ...
            if s["label"].startswith("lane_") and s["label"] != "lane_line"
        ]
        if lane_shapes:
            lane_zones = {}
        ...
        # Auto calibration gate: apply traffic violations only when essentials exist.
        has_stop_line = "stop_line" in scene
        has_traffic_light = "traffic_light_roi" in scene
        valid_lane_keys = {"motorbike", "car", "truck"}
        has_lane_zones = (
            "lane_zones" in scene and
            any(k in valid_lane_keys for k in scene["lane_zones"].keys())
        )
        has_lane_lines = "lane_lines" in scene and len(scene["lane_lines"]) > 0
        scene["calibrated_for_violation"] = (
            has_stop_line and has_traffic_light and has_lane_zones and has_lane_lines
        )
        ...
        def load_yaml_relaxed(path):
            ...
            with open(path, "r", encoding="utf-8") as f:
                txt = f.read()
            # OpenCV style header ("%YAML:1.0") is not standard YAML for PyYAML.
            if txt.lstrip().startswith("%YAML:"):
                lines = txt.splitlines()
                lines = [ln for ln in lines if not ln.startswith("%YAML:")]
                txt = "\n".join(lines)
            try:
        ...
        saved_paths = []
        for out_path in (CONFIG_PATH, DEPLOY_CONFIG_PATH):
            os.makedirs(os.path.dirname(out_path), exist_ok=True)
            existing = load_yaml_relaxed(out_path)
        ...
        cal = "ON" if scene["calibrated_for_violation"] else "OFF"
        self.status(
            "💾 Saved scene to:\n"
            + "\n".join(saved_paths)
            + f"\ncalibrated_for_violation={cal}"
        )
